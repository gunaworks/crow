    ...
    with yaml_output_column:
        ...
        with st.container(height=300, border=True):
            ...
            if submit:
                ...
                with stylable_container("codeblock","""code {white-space: pre-wrap !important;}""",):
        ...
        with repository_link_column:
            repository_link = st.text_input(label="Repository Name",placeholder = "Link to repository...", 
        ...
        if push_to_github:
            if len(st.session_state.yaml_content) and len(repository_link):
                push_yaml_to_github(access_token, yaml_content = st.session_state.yaml_content, repository_link = repository_link)
